...
import { StrategicVibe } from '@/components/StrategicVibe';
...
import {
  ...
  MessageSquare, Mail, Phone, MapPin, Twitter, Linkedin, Loader2
} from 'lucide-react';
import Link from 'next/link';
import { Project, Insight, IntelligenceReport } from '@/lib/types';
import { generateProjectIntelligence } from '@/lib/intelligence-actions';
  ...
  async function handleGenerateReport() {
    ...
    try {
      ...
      if (result.success && result.report) {
        ...
        alert(result.error || 'Failed to generate intelligence report');
      }
    } catch (e) {
      console.error('Report generation error:', e);
      alert('Neural synthesis link failure');
    } finally {
    ...
    <main className="min-h-screen bg-[#050510] text-slate-200 selection:bg-purple-500/30 relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto p-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-12">
          <StatsCard
            title="Global Health"
            value={projects.length > 0 ? projects[0].stats?.healthScore || 88 : 0}
            icon={<Shield size={24} />}
            trend="Active"
          />
          <StatsCard
            title="Sprint Velocity"
            value={activeReport?.velocity.currentVelocity.toFixed(1) || '0.0'}
            trend={activeReport?.velocity.trend === 'increasing' ? '+1.2' : 'Stable'}
            icon={<Target size={24} />}
          />
          <StatsCard
            title="Active Repos"
        ...
        {/* Proactive Vibe Check Section */}
        <StrategicVibe
          score={activeReport?.sentiment.score || 88}
          summary={activeReport?.vibeCheck.summary || "The system is observing operational patterns. Neural link active."}
          focus={activeReport?.vibeCheck.focusArea || "Baseline Analysis"}
          trend={activeReport?.vibeCheck.riskTrend || "stable"}
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-24">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="relative">
              <NeuralCore projects={projects} onClick={handleGenerateReport} />
              <AnimatePresence>
                {generatingReport && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 bg-black/60 backdrop-blur-md rounded-[2.5rem] flex flex-col items-center justify-center gap-4 z-20"
                  >
                    <Loader2 className="w-12 h-12 text-purple-500 animate-spin" />
                    <span className="text-[10px] font-black text-purple-400 uppercase tracking-[0.3em] animate-pulse">Synthesizing Neural Data...</span>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
      ...
      {activeReport?.nudges && <NudgeSystem nudges={activeReport.nudges} />}
    </main>
  );
}
